...
const T = {
  ko: {
    ...
    off: '18% OFF',
    priceOriginal: '₩16,800',
    priceDiscount: '₩13,800',
    yourType: '당신의 애착유형',
  ...
  useEffect(() => {
    if (showPayhipEmbed) {
      ...
      if (!document.getElementById(scriptId)) {
        ...
        // Script already loaded, re-trigger embed rendering
        if (window.PayhipEmbed) {
          window.PayhipEmbed.init();
        }
      }
    }
  }, [showPayhipEmbed, payhipEmbedKey]);
          ...
          {screen === 'result' && info && (
            <div>
              ...
              <div className="bg-gradient-to-br from-rose-50 to-pink-50 rounded-2xl p-6 mb-4 border border-rose-100">
                ...
                <p className="text-xs text-gray-500 mb-4">{t.loveResetSub}</p>
                <div className="flex items-center gap-3 mb-4">
                  <span className="strike text-gray-400 text-sm">{t.priceOriginal}</span>
                  <span className="text-xl font-bold text-gray-800">{t.priceDiscount}</span>
                  <span className="bg-red-500 text-white text-xs font-bold px-2 py-0.5 rounded-full">{t.off}</span>
                </div>
                <button onClick={() => setShowPayhipEmbed(!showPayhipEmbed)}
                ...
                {showPayhipEmbed && (
                  <div className="mt-4 rounded-xl overflow-hidden" style={{ minHeight: '400px' }}>
                    <div className="payhip-embed-page" data-key={payhipEmbedKey}></div>
                  </div>
                )}
              </div>
